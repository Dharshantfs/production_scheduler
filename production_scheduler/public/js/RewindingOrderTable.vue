<template>
  <div class="cc-container">
    <div class="cc-filters">
      <div class="cc-filter-title">Rewinding Order Table</div>
...
function goToBoard() {
  frappe.set_route("rewinding-board");
}
...
async function handleShiftDrop(targetShift) {
  ...
  try {
    const res = await frappe.call({
      method: "production_scheduler.api.assign_rewinding_shift",
    ...
    primary_action: async (vals) => {
      try {
        ...
        if (res?.message?.status === "success") {
          frappe.show_alert({ message: res.message.message || "Rewinding maintenance saved", indicator: "green" }, 4);
          d.hide();
          await fetchMaintenanceRecords();
          await fetchData();
        } else {
...
async function fetchData() {
  ...
  try {
    ...
    const r = await frappe.call({
      method: "production_scheduler.api.get_rewinding_order_table_data",
      args,
    });
...
async function syncSprWeightToTable() {
  try {
    // Fabric qty / child WO qty now comes from live WO+PP data path in get_rewinding_order_table_data.
    // Keep this button as a manual refresh action without heavy backend sync.
    await fetchData();
    frappe.show_alert({ message: "Rewinding table refreshed from live WO data", indicator: "green" }, 4);
  } catch (e) {
    console.error(e);
